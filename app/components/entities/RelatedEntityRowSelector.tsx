...
import { Entity, EntityRow } from "@prisma/client";
...
export interface RefRelatedEntityRowSelector {
  focus: () => void;
}
...
interface Props {
  ...
  entity: Entity;
  items: EntityRow[];
  selected: EntityRow | undefined;
  className?: string;
  onSelected: (item: EntityRow) => void;
}

const RelatedEntityRowSelector = ({ name, className, entity, items, selected, onSelected }: Props, ref: Ref<RefRelatedEntityRowSelector>) => {
                    ...
                    <div className="truncate flex-grow flex justify-between space-x-2">
                      <div className="flex space-x-2 justify-between w-full">
                        <div className="truncate">
                          {EntityRowHelper.getRowFolio(entity, selected)} - {DateUtils.dateYMD(selected.createdAt)}
                        </div>
                        <div className=" flex-shrink-0 font-bold">{EntityRowHelper.getRowDescription(entity, selected)}</div>
                              ...
                              <div className={clsx("truncate flex space-x-2 justify-between items-center w-full", active ? "text-white" : "text-gray-500", "")}>
                                <div className="truncate">
                                  <div className="flex space-x-1 items-center">
                                    <div>{EntityRowHelper.getRowFolio(entity, item)}</div>
                                    <div className=" text-xs">({DateUtils.dateYMD(item.createdAt)})</div>
                                  </div>
                                  <div className="truncate">{EntityRowHelper.getRowDescription2(entity, item)}</div>
                                </div>
                                <div className=" flex-shrink-0 font-bold">{EntityRowHelper.getRowDescription(entity, item)}</div>
                              </div>
                            </div>
                            {selected ? (
...
export default forwardRef(RelatedEntityRowSelector);
