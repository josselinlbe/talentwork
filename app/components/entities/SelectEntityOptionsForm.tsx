...
export interface RefSelectEntityOptionsForm {
  set: (options: EntityPropertyOption[]) => void;
}

interface Props {
  title: string;
  onSet: (items: EntityPropertyOption[]) => void;
}

const SelectEntityOptionsForm = ({ title, onSet }: Props, ref: Ref<RefSelectEntityOptionsForm>) => {
  ...
  const [items, setItems] = useState<EntityPropertyOption[]>([]);

  function set(options: EntityPropertyOption[]) {
    setItems(options);
    if (options.length === 0) {
      addOption();
    }
...
export default forwardRef(SelectEntityOptionsForm);
