...
export default function EntityRowsListAndTable({ entity, items, withTenant = false, withFolio = true, className = "" }: Props) {
  const params = useParams();
  const { t } = useTranslation();

  const [sortByColumn, setSortByColumn] = useState<EntityProperty>();
  ...
  useEffect(() => {
    ...
    if (withFolio) {
      headers.push({ name: "folio", title: t("models.entityRow.folio") });
    }
    entity.properties
      .filter((f) => !f.isHidden)
      .forEach((property) => {
  ...
  const sortedItems = () => {
    ...
    return items.slice().sort((x, y) => {
      const xValue = EntityRowHelper.getPropertyValue(entity, x, sortByColumn);
      const yValue = EntityRowHelper.getPropertyValue(entity, y, sortByColumn);
      if (xValue && yValue) {
  ...
  function getEditRoute(item: EntityRowWithDetails) {
    if (item.tenant) {
                                ...
                                <tr key={idx}>
                                  ...
                                  {withFolio && (
                                    <td className="px-2 py-1 whitespace-nowrap text-sm text-gray-600">
                                      ...
                                      >
                                        {EntityRowHelper.getRowFolio(entity, item)}
                                      </Link>
                                    </td>
                                  )}
                                  {entity.properties
                                    ...
                                    .map((property) => {
                                      return (
                                        <td key={property.name} className="px-2 py-1 whitespace-nowrap text-sm text-gray-600">
                                          {EntityRowHelper.getCellValue(entity, item, property)}
                                          {/* {JSON.stringify(item)} */}
                                        </td>
                                      );
                                    })}
