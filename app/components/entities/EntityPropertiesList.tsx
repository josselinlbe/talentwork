...
interface Props {
  entityId: string;
  items: EntityPropertyWithDetails[];
  className?: string;
  // onUpdated: (items: EntityPropertyWithDetails[]) => void;
}

export default function EntityPropertiesList({ entityId, items, className }: Props) {
  const { t } = useTranslation();
  const submit = useSubmit();

  // const entityPropertyForm = useRef<RefEntityPropertyForm>(null);
  ...
  function create() {
    ...
    // entityPropertyForm.current?.create(nextOrder);
  }

  function update(idx: number, item: EntityPropertyWithDetails) {
    // entityPropertyForm.current?.update(idx, item);
  }

  function updated(item: EntityPropertyWithDetails) {
    // entityPropertyForm.current?.close();
    // setItems(getSortedItems([...items.filter((f) => f.id !== item.id), item]));
  }

  function created(item: EntityProperty) {
    // entityPropertyForm.current?.close();
    // setItems(getSortedItems([...items, item]));
  }

  function deleteField(item: EntityProperty) {
  ...
  function deleted(item: EntityProperty) {
    // setItems(getSortedItems(items.filter((f) => f.id !== item.id)));
  }

  function confirmedDelete(item: EntityProperty) {
    ...
    <div className={clsx(className, "")}>
                    ...
                    <div className="flex items-center space-x-2 justify-between">
                      <div className="flex items-center space-x-2">
                        <div className=" flex items-center space-x-3">
                          <div className="  flex items-center space-x-2">
                            <EntityPropertyBadge type={item.type} className="text-gray-400 h-4 w-4" />
                            {/* <div className="truncate text-sm text-gray-400">{t("entities.fields." + EntityPropertyType[item.type])}</div> */}
                          </div>
      ...
      <div className="">
        <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-2 justify-between">
                    ...
                    <div className="flex space-x-1 flex-shrink-0">
                      <div className="flex space-x-1 truncate p-1">
                        {item.type === EntityPropertyType.SELECT && item.parentId ? (
          ...
          {items.filter((f) => !f.isDefault && f.isDetail).length > 0 && <h3 className="font-medium text-sm">Detail Properties</h3>}
          {items
            .filter((f) => !f.isDefault && f.isDetail)
            .sort((a, b) => a.order - b.order)
            .map((item, idx) => {
      ...
      {/* <EntityPropertyForm ref={entityPropertyForm} entityId={entityId} onCreated={created} onUpdated={updated} onDeleted={deleted} /> */}
...
const PropertyTitle = ({ item }: { item: EntityPropertyWithDetails }) => {
  const { t } = useTranslation();
  return (
    <>
