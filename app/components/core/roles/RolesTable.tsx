        ...
        formattedValue: (i) => (
          <div className="w-40 truncate">
            {tenantId ? (
              ...
              <div>
                {i.permissions.length} <span className="text-xs italic text-gray-400 max-w-sm">({i.permissions.map((f) => f.permission.name).join(", ")})</span>{" "}
              </div>
            )}
          </div>
        ),
    ...
    <div className="space-y-2">
      ...
      <Modal open={permissionsModalOpen} setOpen={setPermissionsModalOpen}>
        <div className="flex items-baseline justify-between space-x-2">
          <h4 className="font-bold text-lg">{selectedRole?.name}</h4>
