...
export default function UserBadge({ item, admin, withEmail = true, withAvatar }: Props) {
  ...
  return (
    <>
      {!withAvatar ? (
        <div>
          {item.firstName} {item.lastName} {withEmail && <span className="italic text-xs font-normal opacity-80">({item.email})</span>}
        </div>
      ) : (
        <div className="flex items-center">
          ...
          <div className="ml-3 truncate">
            ...
            <div className="opacity-80 truncate">{item.email}</div>
          </div>
        </div>
      )}
    </>
