...
export interface RefSelectWorkspaces {
...
interface Props {
  ...
  allowSearch?: boolean;
  onClosed?: () => void;
  onSelected: (workspaces: Workspace[]) => void;
}
...
const SelectWorkspaces = ({ items, maxSize = "sm:max-w-lg", allowSearch, onClosed, onSelected }: Props, ref: Ref<RefSelectWorkspaces>) => {
  ...
  function toggle(item: Workspace) {
    if (isSelected(item)) {
  ...
  function isSelected(item: Workspace) {
  ...
  function accept() {
    ...
    items.forEach((element) => {
      if (isSelected(element)) {
        selectedWorkspaces.push(element);
      }
    });
    if (onSelected) {
      onSelected(selectedWorkspaces);
    }
    close();
  }
  const filteredItems = (items: Workspace[]): Workspace[] => {
    if (!items) {
      return [];
    }
                  ...
                  <div className="max-w-lg mx-auto">
                    <div>
                      <div className="text-center">
                        <h2 className="mt-2 text-lg font-medium text-gray-900">{t("app.workspaces.actions.selectWorkspaces")}</h2>
                        <p className="mt-1 text-sm text-gray-500">{t("app.workspaces.actions.select")}</p>
                      </div>
                      {allowSearch && (
                    ...
                    <div className="mt-5">
                      <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{t("models.workspace.plural")}</h3>
                      {(() => {
                        if (items.length === 0) {
                          return (
                            <div>
...
export default forwardRef(SelectWorkspaces);
