...
export default function NewTenantRelationship({ linksCount }: Props) {
  ...
  const maxTenantRelationships = appData.mySubscription?.maxTenantRelationships ?? 0;
  const maxTenantRelationshipsReached = () => {
    if (!appData.mySubscription) {
      return true;
    } else {
      return maxTenantRelationships > 0 && linksCount >= maxTenantRelationships;
    }
  };

  return (
    <div>
      {maxTenantRelationshipsReached() ? (
        <WarningBanner
          redirect={UrlUtils.currentTenantUrl(params, `settings/subscription`)}
          title={t("app.subscription.errors.limitReached")}
          text={t("app.subscription.errors.limitReachedTenantRelationships", [maxTenantRelationships])}
        />
      ) : (
        <>
          ...
          <div className="py-5">
            <div className="flex justify-end py-3 px-4 lg:px-0 lg:py-0">
              <button
                disabled={maxTenantRelationshipsReached()}
                type="button"
                className={clsx(
                  ...
                  maxTenantRelationshipsReached() && "opacity-80 cursor-not-allowed"
                )}
