...
export interface RefTenantRelationshipSelector {
  select: (link: TenantRelationshipWithDetailsAndMembers) => void;
}

interface Props {
  items: TenantRelationshipWithDetailsAndMembers[];
  className?: string;
  onSelected: (id: string, link: TenantRelationshipWithDetailsAndMembers) => void;
}

const TenantRelationshipSelector = ({ items, className = "", onSelected }: Props, ref: Ref<RefTenantRelationshipSelector>) => {
  ...
  const [selected, setSelected] = useState<TenantRelationshipWithDetailsAndMembers | undefined>(undefined);

  useImperativeHandle(ref, () => ({ select }));
  function select(link: TenantRelationshipWithDetailsAndMembers) {
    if (onSelected) {
          ...
          >
            {(() => {
              if (selected) {
                ...
                return (
                  <span className="w-full inline-flex truncate">
                    <span className="truncate">{t("app.tenantRelationships.actions.selectOne")}...</span>
                  </span>
                );
              }
            })()}
            ...
            <div className="absolute z-10 mt-1 w-full bg-white shadow-xl max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
              ...
              {(() => {
                if (items.length > 0) {
                  ...
                  return <div className="py-2 px-3 text-gray-400 text-sm">{t("app.tenantRelationships.thereAreNo")}</div>;
                } else {
                  return <div></div>;
                }
              })()}
...
export default forwardRef(TenantRelationshipSelector);
