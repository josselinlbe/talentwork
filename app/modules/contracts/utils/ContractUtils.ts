  ...
  contract.members.forEach(async (member) => {
    await sendEmail(
      ...
      "contract-new",
      {
        ...
        tenant_creator: contract.tenantRelationship.createdByTenant.name,
        tenant_provider: contract.tenantRelationship.providerTenant.name,
        tenant_client: contract.tenantRelationship.clientTenant.name,
