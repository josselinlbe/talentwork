...
export const action: ActionFunction = async ({ request, params }) => {
  ...
  const findExistingEmployees = employees.map(async (employee) => {
    ...
    const existing = await getEmployeeByEmail(tenantUrl.tenantId, employee.email);
    if (existing) {
  ...
  try {
    await createEmployees(userInfo.userId, tenantUrl.tenantId, employees);
    await createLog(request, tenantUrl, `Created employees`, employees.map((f) => `${f.firstName} ${f.lastName} (${f.email})`).join(" | "));
